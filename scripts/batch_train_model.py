...
if __name__ == '__main__':
    ...
    for stk_name, features in f_dict.items():
        for yyyy, mm, dd in features:
            ...
            if config.target == Target.mid_p_ret.name:
                ...
                tar1=deepcopy(tar)
                tar = tar.shift(-rolling_rows).reset_index()[[0]].rolling(rolling_rows,min_periods=rolling_rows, center=False,
    ...
    for num in range(4):
        ...
        # X_train=pd.DataFrame(pca.transform(X_train),index=X_train.index,columns=[f'pc{i}' for i in range(pca.n_components_)])
        # X_test=pd.DataFrame(pca.transform(X_test),index=X_test.index,columns=[f'pc{i}' for i in range(pca.n_components_)])
        ...
        if USE_LINEAER or USE_FLAML:
            ...
            _train_stat=lob_stat.stat_pred_error(y_train, y_pred1, name=f'train_trunc{y_train_min:.6f}_stat_{num}')
            stat = pd.concat([stat, _train_stat], axis=1)
            ...
            stat = pd.concat([stat, lob_stat.stat_pred_error(y_test, y_pred_trunc, name=f'test_trunc{y_train_min:.6f}_stat_{num}')], axis=1)
        ...
        if USE_AUTOGLUON:
            ...
            y_train_min=y_train.loc[(y_train!=0).values].min()
            y_pred1.loc[(y_pred1<y_train_min)]=0
            _train_stat=lob_stat.stat_pred_error(y_train, y_pred1, name=f'autogluon_train_trunc{y_train_min:.6f}_stat_{num}')
            stat = pd.concat([stat, _train_stat], axis=1)
            ...
            _test_stat=lob_stat.stat_pred_error(y_test, y_pred_trunc, name=f'autogluon_test_trunc{y_train_min:.6f}_stat_{num}')
            stat = pd.concat([stat, _test_stat], axis=1)
    ...
    all_y_pred_trunc.to_csv(res_root + f"all_y_pred_trunc_{extract_model_name(model)}_{config.target}.csv")
    all_y_true.to_csv(res_root + f"all_y_true_{config.target}.csv")